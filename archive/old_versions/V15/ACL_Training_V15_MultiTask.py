# ============================================================
# ACL Tear Detection — V15.2 Multi-Task Multi-View MRNet
# ============================================================
# V11 AUC: ACL=0.923, Men=0.794, Abn=0.770 (Composite=0.854)
# V14.1 FAILED — broken multi-view approach
# V15   FAILED — features.eval()+checkpoint killed gradients
# V15.1 FAILED — BN stats corrupted by 3 views + random fusion bottleneck
#
# V15.2 fixes:
#   1. Freeze ONLY BN layers (not features.eval()) — ImageNet stats stay clean
#   2. Average view features (not concat) — eliminates random fusion layer
#   3. V11-identical heads (1280->2) — proven architecture
...
NUM_EPOCHS = 50
RANDOM_SEED = 42
MAX_SLICES = 20          # ★ 20 slices × 3 views = 60 total (≈ V11's 40 sagittal)
LR = 1e-4
WEIGHT_DECAY = 0.01
PATIENCE = 10
DROPOUT = 0.3            # ★ Restored from V11 (V14.1 used 0.4)
WARMUP_EPOCHS = 3        # ★ LR warmup for multi-view stability
...
# %% [markdown]
# # Cell 5: Dataset — Multi-View with Augmentation
...
# %% [markdown]
# # Cell 7: Model — V15.2 Multi-View Architecture

# %%
class MRNetV15(nn.Module):
    """Multi-view MRNet — V15.3 architecture.

    Design principle: "V11 + averaged multi-view features"
      1. Shared backbone processes each view separately
      2. BN layers TRAINABLE — must adapt to MRI distribution (not ImageNet)
      3. View features AVERAGED (not concatenated) — no random fusion layer
      4. Heads identical to V11 (1280->2) — proven to work
    """
    ...
    def __init__(self, dropout=0.3):
        ...
        # ★ V11-identical heads: 1280->2 (no fusion bottleneck)
        self.head_acl = nn.Linear(1280, 2)
        self.head_meniscus = nn.Linear(1280, 2)
        self.head_abnormal = nn.Linear(1280, 2)

        # ★ V15.3: Count BN layers to confirm they are TRAINABLE
        bn_count = sum(1 for m in self.modules() if isinstance(m, (nn.BatchNorm2d, nn.BatchNorm1d)))
        trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        total = sum(p.numel() for p in self.parameters())
        print(f"  Backbone: EfficientNet-B0 (ALL layers trainable, {bn_count} BN layers adapting)")
        print(f"  Multi-view: feature averaging (no fusion layer)")
        print(f"  Heads: ACL(1280->2), Meniscus(1280->2), Abnormal(1280->2)")
        print(f"  Dropout: {dropout}")
        print(f"  Params: {trainable:,} trainable / {total:,} total")

    def _extract_view(self, view_slices):
        """Process one view through the backbone and max-pool across slices."""
        feat = self.features(view_slices)              # (S, 1280, h, w)
        pooled = self.pool(feat).flatten(1)            # (S, 1280)
        return torch.max(pooled, dim=0)[0]             # (1280,)
    ...
    def forward(self, sag, cor, axi):
        ...
        # Process each view SEPARATELY through backbone
        feat_sag = self._extract_view(sag)   # (1280,)
        feat_cor = self._extract_view(cor)   # (1280,)
        feat_axi = self._extract_view(axi)   # (1280,)

        # ★ AVERAGE features across views (not concatenate)
        combined = torch.stack([feat_sag, feat_cor, feat_axi]).mean(dim=0)  # (1280,)
        combined = combined.unsqueeze(0)  # (1, 1280)
        combined = self.drop(combined)

        # V11-identical heads
        return (self.head_acl(combined),
                self.head_meniscus(combined),
                self.head_abnormal(combined))

# %%
model = MRNetV15(dropout=DROPOUT).to(device)
print(f"  ★ BN mode at init: training={next(m for m in model.modules() if isinstance(m, nn.BatchNorm2d)).training}")
...
def train_epoch(model, loader, optimizer, device):
    model.train()  # ★ V15.3: ALL layers in train mode, including BN
    # BN must adapt running stats to MRI data (ImageNet stats are wrong for grayscale MRI)
    total_loss = 0
...
no_improve = 0
SAVE_PATH = '/content/drive/MyDrive/dataset/best_acl_model_v15_3.pth'

print(f"Training for up to {NUM_EPOCHS} epochs (patience={PATIENCE})...")
print(f"V15.3: Multi-view, trainable BN, feature averaging, dropout={DROPOUT}, max_slices={MAX_SLICES}")
print(f"       LR warmup: {WARMUP_EPOCHS} epochs, then ReduceLROnPlateau\n")

for epoch in range(NUM_EPOCHS):
    # ★ V15.3: LR warmup for first WARMUP_EPOCHS
    ...
    # Step scheduler on composite val AUC (only after warmup)
    if epoch >= WARMUP_EPOCHS:
...
plt.tight_layout()
plt.savefig('/content/drive/MyDrive/dataset/training_history_v15_2.png', dpi=150)
plt.show()
...
print('\n' + '=' * 60)
print('RESULTS — V15.2 (Multi-View Multi-Task MRNet)')
print('=' * 60)
...
plt.suptitle('V15.2 Multi-View Multi-Task — Confusion Matrices', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('/content/drive/MyDrive/dataset/confusion_matrix_v15_2.png', dpi=150)
plt.show()
